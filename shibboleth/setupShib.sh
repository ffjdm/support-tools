...
# TODO SSL support?
# TODO make Tomcat HTTP port a variable
# TODO make Tomcat HTTPS port a variable
# TODO make Tomcat AJP port a variable
# TODO make heap Xmx a variable (512m for example?)
# TODO clean installation folders (see /tmp)
# TODO put registration steps at the end
...
if [ ! "${IDP_SSL_ENABLED}" = "yes" ]; then
  ...
  sed -i "s/8443/${IDP_TOMCAT_HTTP_PORT}/g" src/installer/resources/metadata-tmpl/idp-metadata.xml
else
...
sudo bash -c "echo 'ShibUserPassAuth {
  edu.vt.middleware.ldap.jaas.LdapLoginModule required
    ...
    serviceCredential=\"'${SHIB_LDAP_ADMIN_BIND_PASSWORD}'\"
  	ssl=\"'$(if [ "${IDP_SSL_ENABLED}" = "yes" ]; then echo "true"; else echo "false"; fi)'\"
    tls=\"'$(if [ "${IDP_SSL_ENABLED}" = "yes" ]; then echo "true"; else echo "false"; fi)'\"
  	userFilter=\"uid={0}\"
    subtreeSearch="true"
  ;
